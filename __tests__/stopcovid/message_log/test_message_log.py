...
class TestMessageLog(unittest.TestCase):
    def test_calls_persistence_with_transformed_commands(self, persistence_mock):
        ...
        self.assertEqual(upserts[0]["from_phone"], commands[0].payload["From"])
        self.assertEqual(upserts[0]["to_phone"], commands[0].payload["To"])
        ...
        self.assertEqual(upserts[1]["from_phone"], commands[1].payload["From"])
        ...
        self.assertEqual(upserts[2]["from_phone"], commands[2].payload["From"])
        self.assertEqual(upserts[2]["to_phone"], commands[2].payload["To"])
